 ...
 * @author Dannz
 * http://www.dannz.net
 * https://twitter.com/dannz
 * http://www.devstate.net
 * Sources for this can be found at:
 * https://github.com/dannz/dannzNetCommonJS
 */


(function (window){

  var Dannz = window.Dannz || {};
  window.Dannz = window.Dannz || Dannz;
    
    Dannz.MathUtil = {};
    
    //used for radiansToDegrees and degreesToRadians
    Dannz.MathUtil.PI_180 = Math.PI/180;
    Dannz.MathUtil.ONE80_PI = 180/Math.PI;
    
    //precalculations for values of 90, 270 and 360 in radians
    Dannz.MathUtil.PI2 = Math.PI*2;
    Dannz.MathUtil.HALF_PI = Math.PI/2;


    //return number between 1 and 0
    Dannz.MathUtil.normalize = function(value, minimum, maximum){
        return (value - minimum) / (maximum - minimum);
    };

    //map normalized number to values
    Dannz.MathUtil.interpolate = function(normValue, minimum, maximum){
        return minimum + (maximum - minimum) * normValue;
    };

    //map a value from one set to another
    Dannz.MathUtil.map = function(value, min1, max1, min2, max2){
        return Dannz.MathUtil.interpolate( Dannz.MathUtil.normalize(value, min1, max1), min2, max2);
    };

    Dannz.MathUtil.getRandomNumberInRange = function(min, max){
        return min + Math.random() * (max - min);
    };
    
    Dannz.MathUtil.getRandomIntegerInRange = function(min, max){
        return Math.round(Dannz.MathUtil.getRandomNumberInRange(min, max));
    };
...
(function (window){
    ...
    Dannz.Geom.Point = function (x,y){
        this.x = isNaN(x) ? 0 : x;
        this.y = isNaN(y) ? 0 : y;
    };

    Dannz.Geom.Point.prototype.clone = function(){
        return new Dannz.Geom.Point(this.x,this.y);
    };

    Dannz.Geom.Point.prototype.update = function(x, y){
        this.x = isNaN(x) ? this.x : x;
        this.y = isNaN(y) ? this.y : y;
    };

    Dannz.Geom.Point.prototype.equals = function(point){
        return this.x==point.x && this.y==point.y;
    };

    Dannz.Geom.Point.prototype.toString = function(){
    ...
    Dannz.Geom.Rectangle = function (x, y, width, height){
        this.update(x, y, width, height);
    };
    
    Dannz.Geom.Rectangle.prototype.update = function(x, y, width, height){
        this.x = isNaN(x) ? 0 : x;
        this.y = isNaN(y) ? 0 : y;
    ...
    Dannz.Geom.Rectangle.prototype.getRight = function(){
        return this.x + this.width;
    };
    
    Dannz.Geom.Rectangle.prototype.getBottom = function(){
        return this.y + this.height;
    };

    Dannz.Geom.Rectangle.prototype.getCenterX = function(){
        return this.x + this.width/2;
    };

    Dannz.Geom.Rectangle.prototype.getCenterY = function(){
        return this.y + this.height/2;
    };

    Dannz.Geom.Rectangle.prototype.containsPoint = function(x, y){
        return x >= this.x && y >= this.y && x <= this.getRight() && y <= this.getBottom();
    };

    
    Dannz.Geom.Rectangle.prototype.clone = function(){
        return new Dannz.Geom.Rectangle(this.x, this.y, this.width, this.height);
    };
    
    Dannz.Geom.Rectangle.prototype.toString = function(){
...
/**
 * Created by dannz on 27-1-14.
 * has a dependecy on Dannz.Geom
 * has a dependecy on Dannz.BitmapUtil
 */
...
(function (window){
    ...
    Dannz.CanvasTextUtil = {};

    //returns the biggest font size that best fits into rect
    Dannz.CanvasTextUtil.getFontSizeForRect = function(string, fontProps, rect, canvas, fillStyle){
        if(!canvas){
    ...
    Dannz.CanvasTextProperties = function(fontWeight, fontStyle, fontSize, fontFace){
    ...
    Dannz.CanvasTextProperties.NORMAL = "normal";
    Dannz.CanvasTextProperties.BOLD = "bold";
    Dannz.CanvasTextProperties.BOLDER = "bolder";
    Dannz.CanvasTextProperties.LIGHTER = "lighter";

    Dannz.CanvasTextProperties.ITALIC = "italic";
    Dannz.CanvasTextProperties.OBLIQUE = "oblique";


    Dannz.CanvasTextProperties.prototype.setFontWeight = function(fontWeight){
        switch (fontWeight){
            case Dannz.CanvasTextProperties.NORMAL:
            case Dannz.CanvasTextProperties.BOLD:
            case Dannz.CanvasTextProperties.BOLDER:
            case Dannz.CanvasTextProperties.LIGHTER:
                this.fontWeight = fontWeight;
                break;
            default:
                this.fontWeight = Dannz.CanvasTextProperties.NORMAL;
        }
    };

    Dannz.CanvasTextProperties.prototype.setFontStyle = function(fontStyle){
        switch (fontStyle){
            case Dannz.CanvasTextProperties.NORMAL:
            case Dannz.CanvasTextProperties.ITALIC:
            case Dannz.CanvasTextProperties.OBLIQUE:
                this.fontStyle = fontStyle;
                break;
            default:
                this.fontStyle = Dannz.CanvasTextProperties.NORMAL;
        }
    };

    Dannz.CanvasTextProperties.prototype.setFontSize = function(fontSize){
    ...
    Dannz.CanvasTextProperties.prototype.clone = function(){
        return new Dannz.CanvasTextProperties(this.fontWeight, this.fontStyle, this.fontSize, this.fontFace);
    };

    Dannz.CanvasTextProperties.prototype.getFontString = function(){
...
function commitResize(){
    ...
    htmlBounds = new Dannz.Geom.Rectangle(0,0, getWidth(canvasContainer) , getHeight(canvasContainer));
    ...
    bounds = new Dannz.Geom.Rectangle(0,0, canvas.width, canvas.height);
    context.clearRect(0,0,canvas.width, canvas.height);
...
//========================
    ...
    var scrollSpeed = 3;
    var score;
    var fontProperties = new Dannz.CanvasTextProperties(Dannz.CanvasTextProperties.BOLD, null, 100);
    ...
    function createLogo(logoText, logoCanvas, logoCanvassBG){
        ...
        var textRect = new Dannz.Geom.Rectangle(0, 0, logoCanvas.width * .8, logoCanvas.height);
        var logoFontProps = fontProperties.clone();
        logoFontProps.fontSize = Dannz.CanvasTextUtil.getFontSizeForRect(logoText, fontProperties, textRect);
    ...
    var collisionPoint = new Dannz.Geom.Point();
    var birdPoints = [];
    ...
    var characters;
    var birdFontProperties = new Dannz.CanvasTextProperties(Dannz.CanvasTextProperties.BOLD, null, 50);
    ...
    function createTubes(){
        ...
        for(var i = 0; i < totalTubes; i++){
            ...
            tubes[i].topRect = new Dannz.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
            tubes[i].bottomRect = new Dannz.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
            renderTube(tubes[i]);
        }
        currentTube = tubes[0];
    }
...
function createCityGraphic(){
    ...
    for(iterator=0; iterator<canvas.width; iterator+=minCloudRadius){
        cgContext.beginPath();
        cgContext.arc( iterator , maxCloudRadius, Dannz.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius), 0, Dannz.MathUtil.PI2);
        cgContext.closePath();
        cgContext.fill();
    }
    ...
    for(iterator=0; iterator<canvas.width; iterator+=(houseWidth+8)){
        ...
        houseHeight = Dannz.MathUtil.getRandomNumberInRange(cityGraphicHeight *.5 , cityGraphicHeight - maxCloudRadius *.8);
    ...
    for(iterator=0; iterator<canvas.width; iterator+=minTreeRadius){
        cgContext.beginPath();
        radius = Dannz.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius)
        cgContext.arc( iterator , cityGraphicHeight, radius, 0, Dannz.MathUtil.PI2);
        cgContext.closePath();
        cgContext.fill();
    ...
    var sand = "#dcd795";
    var groundGraphicRect = new Dannz.Geom.Rectangle();
